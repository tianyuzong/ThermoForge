...
助手支持连续对话。生成后可以在“继续补充或回答问题”中逐条回答，也可以修改材料、边界、热源、时间或网格。每轮保留之前的用户要求、助手追问及最近一次通过校验的草案；明确改口只修改对应参数，不会重复执行前文的新增或删除操作。热源位置无效、必需参数缺失、要求冲突或当前求解器无法表达时，会继续追问，“确认并仿真”保持禁用，直到问题全部解决且配置与几何校验通过。输入新的补充后需先发送并重新校验。

例如，助手问“中心在空腔，是否外置？需要观察多久的冷却过程？”后，可先回复“同意改用外置热源”，再回复“仿真总时长 300 秒，热源仍在 180 秒关闭”。复杂指代和简短选择请使用 Codex；本地规则适合“环境温度 30°C，网格尺寸 3 mm”等明确修改，无法确定时会提示切换 Codex并保留对话。助手提出的可选方案不会自动视为用户同意。
